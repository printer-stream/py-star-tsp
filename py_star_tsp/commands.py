...
def set_led_blink(m: int, n1: int, n2: int) -> bytes:
    """ESC GS L DC1 m n1 n2 — set LED blink."""
    return (
        ESC + GS + b"L" + DC1
        + _byte(m, "m", 0, 255)
        + _byte(n1, "n1", 0, 255)
...
def raster_set_ff_mode(n: int) -> bytes:
    """ESC * r F n NUL — set FF mode."""
    return ESC + b"*rF" + _byte(n, "n", 0, 255) + NUL


def raster_set_page_length(n: int) -> bytes:
    """ESC * r P n NUL — set page length."""
    return ESC + b"*rP" + _byte(n, "n", 0, 255) + NUL


def raster_set_print_quality(n: int) -> bytes:
    """ESC * r Q n NUL — set print quality."""
    return ESC + b"*rQ" + _byte(n, "n", 0, 255) + NUL
...
def raster_transfer_auto_lf(n1: int, n2: int, data: bytes) -> bytes:
    """b n1 n2 data — transfer raster line with automatic LF."""
...
def raster_vertical_move(n: int) -> bytes:
    """ESC * r Y n NUL — vertical move."""
    return ESC + b"*rY" + _byte(n, "n", 0, 255) + NUL
